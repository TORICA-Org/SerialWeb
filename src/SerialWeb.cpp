...
namespace SWNamespace {
  ...
  void SWClass::begin (const IPAddress AP_IP, const byte DNS_PORT) {
    ws.onEvent([](AsyncWebSocket *server, AsyncWebSocketClient *client,
      ...
      instance->handleWsEvent(server, client, type, arg, data, len);
  ...
  int SWClass::available () {
    ws.cleanupClients(maxClients); // クライアントのクリーンアップ
    return strlen((const char *)reading);
  }

  String SWClass::readString () {
    String msg = String((char *)reading);
    reading[0] = '\0';
    return msg;
  }
    ...
    if (type == WS_EVT_CONNECT) { // クライアント接続時
      ...
      if (info->final && info->index == 0 && info->len == len) { // 完全なメッセージが1フレームで届いた場合
        if (info->opcode == WS_TEXT) { // テキストメッセージの場合
          ...
          size_t cplen = min((size_t)len, (size_t)(BUFFER_SIZE - 1));
          memcpy((void *)writing, data, cplen);
          writing[cplen] = '\0';
          reading = writing;
          writing = stash;
        }        
